...
const MapComponent = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const fetchNasaImage = async (lat, lon) => {
      try {
        const response = await axios.get(
          `https://api.nasa.gov/planetary/earth/assets?lon=${lng}&lat=${lat}&dim=0.10&api_key=eDSoPYOM0Z5uzKwS9wjMzTjZbdIMh0g7uYTffSIG`
        );

        // Log the response to debug
        console.log(response.data);

        // Assuming response.data has an image URL, you can set it to state
